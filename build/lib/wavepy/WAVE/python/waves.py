...
#            fgcol = Hlcol
        ...
        widg.configure(fg=fgcol,bg=bgcol,relief=reli)
...
def pmenu(kmenu):
  ...
  if Debug > 0:
    print("Imenu: ",Imenu)
    print("Menu and veto: ",Menu[kmenu][1]," ",MenuVeto[kmenu][2])
    print(Menu[Imenu][0]," ",Menu[Imenu][1]," ",Menu[Imenu][2]," ",Menu[Imenu][3])
  #endif Debug > 0:

  if re.search('CONTINUATION',Menu[Imenu][3]):
    Mtit = Menu[Imenu][3] + '(continued)'
  else:
    Mtit = Menu[Imenu][3]
  #endif re.search('CONTINUATION',Menu[Imenu][3])
  ...
  if iexist == 0:
    ...
    if not PMenuGeo[Imenu]:
      Pmenu[Imenu].geometry(WinPos)
    else:
      Pmenu[Imenu].geometry(PMenuGeo[Imenu])
    #endif not PMenuGeo[Imenu]:
    Pmenu[Imenu].configure(bg = 'yellow')
  ...
  while item <= Menu[Imenu][0]:

    if Debug > 1:
      print(item," ",Menu[Imenu][item])

    if Menu[Imenu][item] == 'TEXT':
      ...
      if Menu[Imenu][4] == 'MENU':

        Iarr = -1

        if Menu[Imenu][itemt] == 'ARRAY':

          Iarr = Menu[Imenu][itemv]
          ...
          SFrame.append(None)
          SFrame[Nsitem] = Frame(Pmenu[Imenu], relief=RIDGE, bd=2,
                                 bg = B_color)

          slab=StringVar()
          slab.set(Menu[Imenu][item])

          if item < nitem - 3:
            if Menu[Imenu][item+3] == 'COMMENT':
              slab.set(Menu[Imenu][item] + '\n' + Menu[Imenu][item+4])
          #endif item < nitem - 3

          slabel = Label(SFrame[Nsitem],text=slab.get(),
                         fg=vcolor, bg=Eb_color,
                         font = MyWavesFont)
          ...
          Entry(SFrame[Nsitem],bd=2,justify=CENTER, \
          font = MyWavesFont, \
          ...
          for i in range(nvars):
            ...
            SFrame.append(None)
            SFrame[Nsitem] = Frame(Pmenu[Imenu],relief=RIDGE,bd=2,bg=B_color)
            ...
            #slabel.bind("<Enter>",lambda event : slabel.configure(fg = Bg_color))
            #slabel.bind("<Leave>",lambda event : slabel.configure(fg = F_color))
        ...
        elif Menu[Imenu][itemt] == 'VARIABLE':

          ivar = Menu[Imenu][itemn]
          ...
          stext=StringVar()
          Menu[Imenu][itemv] = Variables[ivar][1]
          stext.set(Menu[Imenu][itemv])
        ...
        elif Menu[Imenu][itemt] == 'TOGGLE' or \
        Menu[Imenu][itemt] == 'MAPPING' or Menu[Imenu][itemt] == 'BONDING':

          SMitem[Imenu][Nsitem+1][1] = Menu[Imenu][itemt]

          vcolor = T_color
          vstate = 'normal'

          if Menu[Imenu][itemt] == 'TOGGLE':
            ivar = Menu[Imenu][itemn]

            SMitem[Imenu][Nsitem+1][2] = ivar

            sbut = StringVar()
            sbut.set(Menu[Imenu][item])
            ...
            if Variables[ivar][1] == '0':
              SMitem[Imenu][Nsitem+1][0].config(relief=RAISED, bg=B_color)
            else:
              SMitem[Imenu][Nsitem+1][0].config(relief=SUNKEN, bg='red')
          ...
          elif Menu[Imenu][itemt] == 'MAPPING' or Menu[Imenu][itemt] == 'BONDING':

            imap = Menu[Imenu][itemv]
            ...
            SMitem[Imenu][Nsitem+1][0] = Button(Pmenu[Imenu],text=sbut.get(),\
            bg=bgcol,fg=fgcol, \
            activeforeground = hlcol, activebackground = bgcol,\
            highlightcolor = hlcol, highlightbackground = bgcol,\
            font = MyWavesFont, \
            relief=reli,\
            command=\
            lambda kmenu=Imenu,kitem=item,kvar=ivar: ToggleVar(kmenu,kitem,kvar))
          ...
          #endif Menu[Imenu][itemt] == 'TOGGLE':

        #endif Menu[Imenu][itemt] == 'VARIABLE' etc.:

      #endif Menu[Imenu][4] == 'MENU'

#error      elif Menu[Imenu][itemt] == 'SELECT':
      elif Menu[Imenu][4] == 'SELECT':
        ...
        #endif Menu[Imenu][itemv]  != 0:

        SFrame.append(SMitem[Imenu][Nsitem+1][0])
        SFrame[Nsitem].pack(fill='x', padx=PadX, pady=PadY)

      # endelif Menu[Imenu][4] == 'SELECT':

    #endif Menu[Imenu][item] == 'TEXT':

    elif Menu[Imenu][item] == 'COMMENT':

      pass # already done
    #endif Menu[Imenu][item] == 'COMMENT':

    elif Menu[Imenu][item] == 'MENU' or Menu[Imenu][item] == 'SELECT':
      ...
      SMitem[Imenu][Nsitem+1][0] = \
      Button(Pmenu[Imenu],text=Menu[idaughter][3], \
      bg=bgcol, fg=fgcol, \
      activeforeground = afgcol, activebackground = abgcol,\
      highlightcolor = hlcol, highlightbackground = hlbcol,\
      font = MyWavesFont, \
      command=\
    ...
    #endif Menu[Imenu][item] == 'MENU/SELECT':
  ...
  BottomFrame = Frame(Pmenu[Imenu], bd=2, bg=Bg_color)
  ...
  if Help[Imenu][0] > 0:
    ...
    lambda kmenu=Imenu: HelpText(kmenu), bg=B_color,fg=F_color,\
      activeforeground = Bg_color, activebackground = B_color,\
      font = MyWavesFont, \
      highlightcolor = Bg_color, highlightbackground = B_color)
  ...
  OKButton = Button(BottomFrame,text='OK', name='bOkButton',bg='green',fg=F_color,\
  activeforeground = Bg_color, activebackground = 'red',\
  highlightcolor = Bg_color, highlightbackground = 'green',\
  font = MyWavesFont, \
  command=DestroyMenu)
  ...
  #  else:
  #    pass
  #    #print(SMitem[Imenu][Ifocus][0])
  #    SMitem[Imenu][Ifocus][0].focus_set()
  #  #endif Ifocus == -1

#  try:
#    if Ifocus != -1: SMitem[Imenu][item][0].focus_set()
#  except:
#    try:
#      if Ifocus != -1: SMitem[Imenu][-1][0].focus_set()
#    except:
#      pass
#  #endtry
...
def getmother(iknown,isame):
  ...
  if not isame:
    ...
    if Menu[IMother][4] == 'MENU':
      Menu[IMother].append('MENU')
    elif Menu[IMother][4] == 'SELECT':
      Menu[IMother].append('SELECT')

    Menu[IMother].append(mmenu)
    Menu[IMother][0]+= 1
...
#            Wexit('')
      ...
      # endif MENUALLVETO
        ...
        Menu[Nmenu].append('TEXT') #BreakVariable
        Menu[Nmenu][0]+= 1

        Menu[Nmenu].append(text)
        Menu[Nmenu][0]+= 1

        if key == '$VARIABLE':
          Menu[Nmenu].append('VARIABLE')
        elif key == '$ARRAY':
          Menu[Nmenu].append('ARRAY')
        else:
...
# 6: array to control, 7: line number, i.e. index in Wavein])
        ...
        Menu[Nmenu].append('TEXT') #BreakText
        Menu[Nmenu][0]+= 1
        ...
        if key == '$VARIABLE':
          ...
          Menu[Nmenu].append('TOGGLE')
        elif key == '$MAPPING':
          Menu[Nmenu].append('MAPPING')
        elif key == '$BONDING':
          Menu[Nmenu].append('BONDING')
        else:
          print("*** Bad key", key, " for $TEXT ",text," ***")
          Wexit('')
        #endif key == '$VARIABLE'
...
#          Quit("Baustelle")
        ...
        if ifound == -1:
          print('*** Bad goal ',goal,' of trigger ',trig,' ***')
          Wexit('')
...
def Calculate():
  ...
  while i <= Ncalc:
    ...
    if Calc[i][6] == 'VARIABLE':
      ...
      if typr == 'I':
        if res.real >= 0: res = floor(res.real)
        else: res = ceil(res.real)
        res = str(res)
...
# If itrig is set, corresponding menu is triggered.
  ...
  itemt = kitem + 1
  if kitem+2 >= len(Menu[kmenu]): return

  typ = Menu[kmenu][kitem+1]

  if typ == 'MAPPING' or typ == 'BONDING':
    kmap = Menu[kmenu][kitem + 2]
    mp = Mapping[kmap]
    ivar = abs(mp[1][1])
...
#+self.
  ...
  if Menu[kmenu][itemt] == 'MAPPING' or Menu[kmenu][itemt] == 'BONDING':
    if Mapping[kmap][2]:
      m = 0
  ...
  #endif Menu[kmenu][itemt] == 'MAPPING' or Menu[kmenu][itemt] == 'BONDING':
...
def CheckButtons():
  ...
  while i <= Nsitem :
    ...
    if SMitem[M][i][1] == 'MENU' or SMitem[M][i][1] == 'SELECT':
      ...
      if SMitem[M][i][3]:
        SMitem[M][i][0].configure(fg= Fgcol, bg=Select_color)
      elif SMitem[M][i][2]:
        SMitem[M][i][0].configure(fg= Vfgcol, bg=Vbgcol)
      else:
#        print("CheckButton: ",i," ",SMitem[M][i])
        SMitem[M][i][0].configure(fg= Fgcol, bg=B_color)
      #endif SMitem[M][i][3]
      ...
      if Variables[ivar][1]:
        SMitem[M][i][0].configure(fg=vcolor,state=vstate,bg=selcol)
      else:
        SMitem[M][i][0].configure(fg=vcolor,state=vstate,bg=selcol)
      #endif Variables[ivar][1]
      ...
      if Variables[ivar][1]:
        SMitem[M][i][0].configure(fg=vcolor,state=vstate,bg=selcol,relief=SUNKEN)
      else:
        SMitem[M][i][0].configure(fg=vcolor,state=vstate,bg=selcol,relief=RAISED)
      ...
      SMitem[M][i][0].configure(bg=bgcol,fg=fgcol, relief=reli)

    #endif SMitem[M][i][1] == 'MENU':

#    widfoc = GetFocusWidget(M)
#    if widfoc and widfoc == SMitem[M][i][0]:
#      print(SMitem[M][i])
...
def FocusIn(event,kmenu,kitem,kvar):
  ...
  if ivar == 0: ew.configure(fg=Afgcol)

  if kvar == -11:
    ew.configure(fg=Afgcol)
    event.widget.focus()
    return
  elif kvar == -12:
    ew.configure(fg=Afgcol,bg=Hlbcol)
    event.widget.focus()
    return
  #endif kvar == -12
...
# This function updates Variables[...]
  ...
  if kvar == -12:
    ew.configure(fg=Fgcol,bg='green')
    return
  elif kvar == -11:
    ew.configure(fg=F_color)
    return
  #endif kvar == -12
  ...
  if key != '??' and kmenu >= 0:
    ...
    if key == 'Return' or key == 'Tab':
      ...
      Menu[kmenu][kitem] = val

      Variables[kvar][3] = Variables[kvar][1]
      Variables[kvar][1] = Menu[kmenu][kitem]
